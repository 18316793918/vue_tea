        ...
        connection.query(`select * from goods_list where id=${goodsId}`,function(err,result){
            ...
            connection.query(`select * from goods_cart where uId=${uId} and goods_id=${goodsId}`,function(e,r){
                ...
                if( r.length > 0 ){
                    ...
                    //没有
                    connection.query(`insert into goods_cart (uId,goods_id,goods_name,goods_price,goods_num,goods_imgUrl) values ("${uId}","${goodsId}","${goodsName}","${goodsPrice}","1","${goodsImgUrl}")`,function(error,result){
                    res.send({
                           data:{
                               code:200,
                               success:true,
...
router.post('/api/code',function(req,res,next){
	...
	// var appid = 1400641329;  // SDK AppID是1400开头
	
	// 短信应用SDK AppKey
	var appkey = "dc9dc3391896235ddc2325685047edc7";
	// var appkey = "1400641329";
...
//搜索框下拉提示
router.post('/api/searchData',function(req,res) {
	let CurrentVal = req.body.CurrentVal;
	connection.query('select name from goods_list where name like' + '"%' + CurrentVal + '%"',function(err,result) {
		res.send({
			code:200,
			data:{
				result
...
router.get('/api/goods/shopList',function(req,res,next){
	...
	if(orderName == 'zh') {
		connection.query('select * from goods_list where name like' + '"%' + name + '%"',function(err,result) {
			res.send({
				code:200,
				data:result
			})
